...
TYPES : BEGIN OF zauct_struct,
          mandt             TYPE  mandt,
          session_id        TYPE  zdb_analysis_v75-session_id,
          pgname(40)        TYPE  c,
          error_type(3)     TYPE  c,
          ...
          info(50)          TYPE  c,
          repcfm(8000)      TYPE   c,
          obj_name          TYPE zdb_analysis_v75-obj_name,
...
*{ Begin of change by Twara - 12/02/2016
          sub_type          TYPE zdb_analysis_v75-sub_type,
...
TYPES: BEGIN OF ty_correction_results,
         obj_name    TYPE zdb_analysis_v75-obj_name,
         sub_program TYPE zdb_analysis_v75-sub_program,
         line_org    TYPE zdb_analysis_v75-line_no,
         line_no     TYPE zdb_analysis_v75-line_no,
         opcode      TYPE zdb_analysis_v75-opercd,
*{ Begin of change by Twara - 12/02/2016
         message     TYPE string,
*} End of change by Twara - 12/02/2016
...
DATA : gv_sessionc TYPE zdb_analysis_v75-session_id.
** begin of hana check
DATA : it_hana       TYPE STANDARD TABLE OF zdb_analysis_v75,
       wa_hana       TYPE zdb_analysis_v75,
*       begin of change by vrishti - 24/1/2017: Def_2
       wa_hana_check TYPE zdb_analysis_v75,
*       end of change by vrishti - 24/1/2017: Def_2
*{ Begin of change by Twara - 12/02/2016
       wa_hana1      TYPE zdb_analysis_v75,
*} End of change by Twara - 12/02/2016
*{ Begin of Change by Rohit - 24/02/2016
       it_hana_corr  TYPE STANDARD TABLE OF zdb_analysis_v75,
*} End of change by Rohit - 24/02/2016
       it_dp_hana    TYPE STANDARD TABLE OF zdb_analysis_v75,
       wa_dp_hana    TYPE zdb_analysis_v75,
...
CLASS lcl_hana_corrections IMPLEMENTATION.
  ...
  METHOD fetch_data.
    ...
    DATA: lt_r_opcodes  TYPE RANGE OF zdb_analysis_v75-opercd,
          lwa_r_opcodes LIKE LINE OF lt_r_opcodes.
**************************************************
*Taking values from ZDB_ANALYSIS_V75 and putting it
...
***********BOC Def_31 Shreeda 1/05/2017************
      ...
      APPEND lwa_r_opcodes TO lt_r_opcodes.
***********EOC Def_31 Shreeda 1/05/2017************
      "BOC changes for mandatory opcode 17/05
...
*      APPEND lwa_r_opcodes TO lt_r_opcodes.
    ...
    IF s_sess IS NOT INITIAL AND s_obj IS INITIAL.
      SELECT * FROM zdb_analysis_v75 INTO TABLE it_hana WHERE
...
*            wa_hana-detected = 'X'.
**{ Begin of Change by Rohit - 09/02/2016
**            MODIFY ZDB_ANALYSIS_V75 FROM wa_hana .
...
***********BOC Def_31 Shreeda 3/05/2017************
           OR wa_zauct_errors-opercd = 77 OR
            wa_zauct_errors-opercd = 78
***********EOC Def_31 Shreeda 3/05/2017************
            )
            AND  wa_zauct_errors-runn = ''.
  ...
  METHOD display_result.
    ...
    IF NOT i_issue[] IS INITIAL.
      ...
      LOOP AT i_issue INTO wa_issue.
        ...
        CLEAR : lwa_final,wa_issue. " Code change for def_28
      ENDLOOP.
    ENDIF.
    ...
    IF NOT gt_skipped_code IS INITIAL.
      ...
      LOOP AT gt_skipped_code INTO wa_skipped_code.
        ...
        lwa_final-scol = lt_color.
        APPEND lwa_final TO lt_final.
        CLEAR : lwa_final,wa_skipped_code."code change for def_28
      ENDLOOP.
    ENDIF.
    ...
    IF NOT gt_correction_results IS INITIAL.
      ...
      APPEND lwa_color TO lt_color.
      LOOP AT gt_correction_results INTO wa_correction_results.
*        begin of code change for def_28
        READ TABLE i_issue INTO wa_issue WITH KEY name = wa_correction_results-sub_program
                                                  .
        IF sy-subrc EQ 0.
          CONTINUE.
        ELSE.
          READ TABLE  gt_skipped_code INTO wa_skipped_code WITH KEY name = wa_correction_results-sub_program
                    .
          IF sy-subrc EQ 0.
            CONTINUE.
          ENDIF.
        ENDIF.
*        end of code change for def_28
  ...
  METHOD check_opcode12.
    ...
    obj_name = wa_zauct_errors-pgname
    sub_program = wa_zauct_errors-sub_program
...
*****begin of changes by Vrishti - 24/1/2017 : Def_2
  METHOD check_hana_opcode.
    ...
    DATA: rtab1          TYPE TABLE OF ty_rtab1,
          ...
          wa_hana_41     TYPE zdb_analysis_v75,
          lv_last_idx    TYPE i,
          lv_idx         TYPE i,
          lv_idx_astpp   TYPE i, "added by Harman on 02.05.2017 DEF_8
          lv_new(1000)   TYPE c,
...
**************BOC defect reported by Arushi, corrected by shreeda***********
*    LOOP AT it_hana INTO wa_hana_check
*      WHERE obj_name = lv_prog AND
*          line_no = line_check AND
*          session_id IN s_sess .
READ TABLE it_hana INTO wa_hana_check WITH KEY
    session_id = wa_zauct_errors-session_id
    OBJ_name = wa_zauct_errors-pgname
    sub_program = wa_zauct_errors-sub_program
    opercd = wa_zauct_errors-error_type
    ...
    line_no = wa_zauct_errors-line_no.
if sy-subrc eq 0.
**************EOC defect reported by Arushi, corrected by shreeda***********
...
******************
            ...
            IF v_str CS ca_dotpp.
              ...
              "Begin of change by Harman on 02.05.2017          DEF_8
              FIND FIRST OCCURRENCE OF ca_astpp IN wa_rtab-text MATCH OFFSET lv_idx_astpp.
              IF l_times1 > 1.
                DELETE i_result1 WHERE offset GE lv_idx_astpp.
                DESCRIBE TABLE i_result1 LINES l_times1.
                READ TABLE i_result1 INTO wa_result INDEX l_times1.
                pos_dot = wa_result-offset.
                line1 = wa_result-length.
                CLEAR: wa_result, lv_idx_astpp, l_times1.
              ELSEIF l_times1 = 1.
                "End of change by Harman on 02.05.2017            DEF_8
                READ TABLE i_result1 INTO wa_result INDEX l_times1.
                pos_dot = wa_result-offset.
                line1 = wa_result-length.
                CLEAR: wa_result.
              ENDIF. "change for DEF_8
...
*        ENDIF.
          READ TABLE it_hana into wa_hana_check WITH KEY
            ...
            IF sy-subrc = 0.
              wa_hana_check-runn = 'X'.
              wa_hana_check-detected = 'X'.
              wa_hana_check-actual_corr = 'Y'.
              MODIFY zdb_analysis_v75 FROM wa_hana_check.
              ENDIF.
**************EOC defect reported by Arushi, corrected by shreeda********
        ...
        LOOP AT rtab INTO wa_rtab FROM p_line.
          ...
          ELSE.
            ...
            IF v_str CS ca_dotpp.
              ...
              APPEND wa_rtab TO lt_code.
*& Begin of Changes by Arzoo Tomar Def_32 (OpCode 41) &*
** Select End Select is not working fine
*                            EXIT.
              DATA: lv_endselect TYPE char1.
              IF lv_endselect IS INITIAL
             AND lv_code NS 'INTO TABLE'.
                lv_endselect = 'X'.
                CONTINUE.
              ELSEIF lv_code CS 'ENDSELECT'.
                CLEAR lv_endselect.
                EXIT.
              ENDIF.
              IF lv_endselect IS INITIAL.
                EXIT.
              ENDIF.
*& End of Changes by Arzoo Tomar Def_32 (OpCode 41) &*
            ELSE.
              CONCATENATE lv_code v_str INTO lv_code SEPARATED BY space.
              APPEND wa_rtab TO lt_code.
            ENDIF.
        ...
        IF lv_flag_opcode41 EQ 'X'.
          ...
          IF sy-subrc = 0.
            ...
            IF sy-subrc = 0.
              ...
              MODIFY zdb_analysis_v75 FROM <fs_hana>.
            ENDIF.
...
*{ Begin of Change by Rohit - 09/02/2016
    ...
    "/ Modify ZDB_ANALYSIS_V75 Table
    MODIFY zdb_analysis_v75 FROM TABLE it_hana_corr.
*} End of change by Rohit - 09/02/2016
    COMMIT WORK.
...
*----------BOC NLE issue 1 Def_34 Shreeda----------****
    CONDENSE wa_rtab-text.
*----------EOC NLE issue 1 Def_34 Shreeda----------****
...
*        CLEAR : lv_tlen , lv_len , lv_len1 , lv_tlen1.
*****--BOC NLE Def_34 issue 3 by shreeda -------------**********
        CLEAR : lv_len , lv_len1 , lv_tlen1.
*****--EOC NLE Def_34 issue 3 by shreeda -------------**********
        IF lv_flag1 = 'X'.
          lv_code = 'EXIT.'.
          APPEND lv_code TO rtab1.
          lv_code = 'ENDSELECT.'.
